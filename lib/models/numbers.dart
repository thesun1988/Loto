...
class Numbers extends ChangeNotifier {
  ...
  //     growable: false);
  final List isCrossed = List.generate(25, (int index) => false);
  List randomNum = List.generate(50, (int index) => index+1)..shuffle();
  final List x = [];
  final List x1 = [];
  final List x2 = [];
  final List x3 = [];
  ...
  void calX() {
    if (randomNum[count] > 0 && randomNum[count] < 10) x.add(randomNum[count]);
    if (randomNum[count] > 9 && randomNum[count] < 20) x1.add(randomNum[count]);
    if (randomNum[count] > 19 && randomNum[count] < 30)
      x2.add(randomNum[count]);
    if (randomNum[count] > 29 && randomNum[count] < 40)
      x3.add(randomNum[count]);
    if (randomNum[count] > 39 && randomNum[count] < 51)
      x4.add(randomNum[count]);
    x.sort();
    x1.sort();
    x2.sort();
    x3.sort();
